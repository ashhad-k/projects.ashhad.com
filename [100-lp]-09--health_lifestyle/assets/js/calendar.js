...
class CalendarYvv{
	constructor(etiqueta="", diaSeleccionado="", primerDia="Lunes") {
		...
		this.colorResal = "#fdb825"; // color de los dias importantes
		this.textResalt = "#000"; // color de texto de dias importantes

		this.bg = "bg-lite"; // color de fondo de la cabecera
		this.textColor = "text-dark"; // color de texto en la cabecera
		this.btnH = "btn-light"; // color de boton normal
	...
	createDayTag(){
		...
		var cont = $("<div class='d-flex border w-100 border-0 cr-row2 "+this.bg+" "+this.textColor+"'>");

		diasOrd.fechCort.forEach(function(e){
			var div = $("<div class='d-flex flex-fill w-100 cr-row2a justify-content-center p-3'>").html(e);
			cont.append(div);
		});
		return cont;
	}
	createDaysMont(){
		...
		var cont = $("<div class='d-flex border w-100 border-0 '>");
		var emptyTag = "<div class='d-flex border flex-fill w-100 justify-content-center rounded-0 pt-5 pb-5 btn cr-item1b' style='color:transparent; background:rgba(255,255,255,0.70)'>";
		...
		for(var i=0;i<this.cantDias;i++){
			...
			var div = $("<div class='d-flex border flex-fill w-100 cr-item1 justify-content-center pt-5 pb-5 btn "+this.btnD+"' data-date='"+fechNow+"'>").html(i+1);
			var clas_e = this;
			var _ind = (this.cantDias+posMes)%7;

			//dia seleccionado
			if(this.diaSeleccionado==fechNow){
				div = $("<div class='current-date-selected d-flex border  cr-item1b  flex-fill w-100 justify-content-center pt-5 pb-5 btn "+this.btnD+"' data-date='"+fechNow+"'>").html(i+1);
			}
			//dias resaltados o importantes
			if(this.diasResal.indexOf(i+1)!=-1){
				div = $("<div class='d-flex border flex-fill w-100  cr-item1a justify-content-center pt-5 pb-5 btn "+this.btnD+"' data-date='"+fechNow+"' style='background: "+this.colorResal+"; color: "+this.textResalt+"; font-weight: bold;'>").html(i+1);
			}
			...
			if(cnt==6){
				...
				cntG.append(cont);
				cont = $("<div class='d-flex border w-100 border-0'>");
				cnt = 0;
